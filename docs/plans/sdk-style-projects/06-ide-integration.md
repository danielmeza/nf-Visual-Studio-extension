...
**Gate note.** The maintainer attributes the VS-side block to the **debugger**
(doc 09 §9.5, discussion
[#1635](https://github.com/orgs/nanoframework/discussions/1635)). A code read
refines this: the project system is already CPS and the deploy/debug-launch
providers key off a capability and launch the engine **by GUID**, so the concrete
gate is the **build-targets composition + capability registration**, with the AD7
engine likely orthogonal. The A+C POC ([poc-sdk-style-debugging-plan.md](poc-sdk-style-debugging-plan.md)) tests
that. The **VS Code / CLI** build path is not gated and can move now.
...
calls an MSBuild target** — but getting there in VS waits on the debugger gate.

## 6.2 Flavor GUID → CPS (gated)
...
Sdk="...">`. This is the single largest IDE change — and the one the debugger
gates, because the current debug/F5 path is wired to the old project system. Until
the gate lifts (doc 09 §9.5), VS continues to load and debug projects through the
legacy flavor.

When the gate lifts, the extension registers a small **CPS capability** keyed off
the nanoFramework SDK (detecting `nanoFramework.Sdk` or
`TargetFrameworkIdentifier=.NETnanoFramework`) to light up Device Explorer /
Deploy / Debug on SDK-style projects.

## 6.3 What can move now vs. behind the gate

| Now (unblocked) | Behind the debugger gate |
|-----------------|--------------------------|
...
| Test Explorer via `dotnet test` | Retiring the flavor project system |
...
## 6.5 The deploy button → MSBuild target (VS, behind the gate)
...
SDK target). It rides on the same CPS/debugger work, so it lands with the gate.
...
project-file format. So the assessment is that the engine is **orthogonal** to
SDK-style; what gates VS debugging is the project loading with the
`NanoCSharpProject` capability (targets composition + registration), not the
engine.
